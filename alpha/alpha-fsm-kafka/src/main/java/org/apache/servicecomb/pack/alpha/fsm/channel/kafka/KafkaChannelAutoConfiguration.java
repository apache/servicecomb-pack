...
public class KafkaChannelAutoConfiguration {
    ...
    @Bean
    @ConditionalOnMissingBean
    public KafkaMessageListener kafkaMessageListener(@Lazy @Qualifier("actorEventSink") ActorEventSink actorEventSink, @Lazy @Qualifier("nodeStatus") NodeStatus nodeStatus){
        return new KafkaMessageListener(actorEventSink, nodeStatus);
    }
}