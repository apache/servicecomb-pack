...
public class AlphaIntegrationTest {
    ...
    public void sagaEndedEventIsAlwaysInTheEnd() throws Exception {
        ...
        await().atMost(1, SECONDS).until(() -> {
            ...
            return events.size() == 6 && events.get(events.size() - 1).type().equals(SagaEndedEvent.name());
        });
    }
    ...
    public void abortTimeoutTxStartedEvent() {
        ...
        await().atMost(2, SECONDS).until(() -> {
            List<TxEvent> events = eventRepo.findByGlobalTxId(globalTxId);
            return eventRepo.count() == 4 && events.get(events.size() - 1).type().equals(SagaEndedEvent.name());
        });
